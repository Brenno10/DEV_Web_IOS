...
function umbrella() {
    if (document.querySelector('#existingImage') !== null) {
        alert('Ja existe uma imagem');
    } else {
        alert('Imagem da Série Umbrella Academy');
        let figureSect = document.createElement('section');
        document.body.appendChild(figureSect);
        figureSect.classList = 'cleanPageInfo';
        figureSect.id = 'existingImage';
...
function requiredName() {
    ...
    if (user.trim() != '') {
        ...
        welcomeSec.classList = 'cleanPageInfo';
    ...
    if (base.length === 0) {
        ...
        if (!isNaN(base)) {
            if (base != 0) {
                ...
                multSect.classList = 'cleanPageInfo';
...
function cleanPage() {
    let cleanPageInfo = document.querySelectorAll('.cleanPageInfo');

    if (cleanPageInfo.length !== 0) {
        for (let i = 0; i < cleanPageInfo.length; i++) {
            cleanPageInfo[i].remove();
        }
